...
class SPUSaleAttrAdmin(BaseModel):
    ...
    def save_model(self, request, obj, form, change):
        ...
        redis_conn.delete('index_cache')
        try:
            # 删除goods_spuid下的商品sku缓存
            redis_conn.delete(*redis_conn.keys("goods_%s_*" % obj.SPU_id.id))
...
class SKUAdmin(BaseModel):
    ...
    def save_model(self, request, obj, form, change):
        super().save_model(request, obj, form, change)
        redis_conn.delete('index_cache')
        redis_conn.delete("goods_%s_%s" % (obj.SPU_ID.id, obj.id))
        print("保存数据时，首页缓存删除，详情页缓存删除")
...
class SKUSaleAttrValueAdmin(BaseModel):
    ...
    def save_model(self, request, obj, form, change):
        super().save_model(request, obj, form, change)
        # 删除详情页缓存
        redis_conn.delete("goods_%s_%s" % (obj.sku.SPU_ID.id, obj.sku.id))
...
class SKUImageAdmin(BaseModel):
    ...
    def save_model(self, request, obj, form, change):
        super().save_model(request, obj, form, change)
        # 删除详情页缓存
        redis_conn.delete("goods_%s_%s" % (obj.sku_id.SPU_ID.id, obj.sku_id.id))
